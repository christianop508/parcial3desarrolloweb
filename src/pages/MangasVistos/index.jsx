...
function MangasVistos() {
  ...
  const agregarManga = async () => {
    ...
    await axios.post('http://localhost:3001/api/mangasvistos', {
  ...
  const eliminarManga = async (id) => {
    await axios.delete(`http://localhost:3001/api/mangasvistos/${id}`);
    fetchMangas();
  };

  const irADetalle = (id) => {
    navigate(`/mangasvistos/${id}`);
  };

  return (
    <div className="min-h-screen bg-gray-900 text-white p-6">
      <h1 className="text-3xl font-bold mb-6 text-center">📖 Mangas Vistos</h1>
      ...
      <div className="flex flex-col items-center gap-4 mb-6">
        ...
        <button
          ...
          className="bg-blue-600 hover:bg-green-700 text-white font-semibold px-4 py-2 rounded"
        >
          Agregar
        </button>
      </div>
